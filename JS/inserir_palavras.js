function inserirPalavras(){
    ...
    const max_min = [
        Math.round((quant_palavras/2)-2), // minimo
        Math.round((quant_palavras/2)-1), // medio
        Math.round(quant_palavras/2) // maximo
    ] 

    let num_horizontais = Math.floor(Math.random() * max_min.length)
    let total_horizontais = max_min[num_horizontais]
    let total_verticais = quant_palavras - total_horizontais
    ...
    function insertHorizontal(palavra){
        ...
        while(true){
            horizontal_x = Math.floor(Math.random() * (comprimento - tamanho_palavra_h)) // pega um index aleatório na linha, mas a palavra inteira precisa caber
            horizontal_y = Math.floor(Math.random() * altura) // pega um index aleatório nas colunas
            let valido1 = 0
            ...
            for(let t1 = 0; t1 < tamanho_palavra_h; t1++){
                if(quadro_usado[horizontal_y][horizontal_x+t1] == null || quadro_usado[horizontal_y][horizontal_x+t1] == letras_h[t1]){
                    valido1++
                }
            }
            ...
            if(valido1 === tamanho_palavra_h){
                for(let h1 = 0; h1 < tamanho_palavra_h; h1++){
                    quadro[horizontal_y][horizontal_x+h1] = letras_h[h1] 
                    quadro_usado[horizontal_y][horizontal_x+h1] = letras_h[h1] 
                }
                palavras_inseridas.push(palavra)
                break
            }
            ...
            if(max_tentativas_h > 0){
                ...
                palavras_nao_usadas.push(palavra)
                break
            }
    ...
    function insertVertical(palavra){
        ...
        // se for impossível o quadro não será gerado nunca
        while(true){
            vertical_x = Math.floor(Math.random() * comprimento )
            vertical_y = Math.floor(Math.random() * (altura - tamanho_palavra_v))
            let valido2 = 0 
            ...
            for(let t2 = 0; t2 < tamanho_palavra_v; t2++){
                if(quadro_usado[vertical_y+t2][vertical_x] == null || quadro_usado[vertical_y+t2][vertical_x] == letras_v[t2]){
                    valido2++ 
                }
            }
            ...
            if(valido2 === tamanho_palavra_v){
                for(let v1 = 0; v1 < tamanho_palavra_v; v1++){
                    quadro[vertical_y+v1][vertical_x] = letras_v[v1] 
                    quadro_usado[vertical_y+v1][vertical_x] = letras_v[v1] 
                }
                palavras_inseridas.push(palavra) 
                break
            } 
    ...
    for(let plv = 0; plv < palavras.length; plv++){
        getWordPosition(palavras[plv])
    }
}